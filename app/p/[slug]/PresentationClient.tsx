  ...
  return (
    <PartnershipProvider>
      <main className="relative">
        ...
        {/* GolfN G indicator nav - right rail */}
          ...
          {sections.map((s, i) => {
            return (
              <a key={s.id} href={`#${s.id}`} className="group flex items-center gap-5 justify-end">
                <span className="text-base font-medium text-[#52525B] opacity-0 group-hover:opacity-100 translate-x-3 group-hover:translate-x-0 transition-all duration-300 whitespace-nowrap">{s.label}</span>
                <span className="block w-3 h-3 rounded-full bg-[#2A2A2C] group-hover:bg-[#52525B] transition-colors shrink-0" />
              </a>
            )
          })}
        </nav>
        ...
        <footer className="w-content px-5 md:px-12 py-12 md:py-16 text-center border-t border-[#2A2A2C]">
          ...
          <p className="text-[#71717A] text-sm">Confidential &mdash; Prepared for {partner.partnerName} by GolfN</p>
        </footer>
      </main>
    </PartnershipProvider>
  )
