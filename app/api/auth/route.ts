...
const SLACK_CHANNEL_ID = 'C0AM2N65ZHT'
...
async function postSlackLogin(email: string, slug: string, ip: string): Promise<string | null> {
  ...
  try {
    const res = await fetch('https://slack.com/api/chat.postMessage', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${SLACK_BOT_TOKEN}` }, body: JSON.stringify({ channel: SLACK_CHANNEL_ID, text: `\uD83D\uDD13 *Partner Login*\n>*Email:* ${email}\n>*Proposal:* \`${slug}\`\n>*Time:* ${timeStr} EST\n>*IP:* ${ip}` }) })
    const data = await res.json()
    if (data.ok && data.ts) return data.ts
    console.error('[AUTH] Slack postMessage error:', data.error)
    return null
  } catch (err) { console.error('[AUTH] Slack postMessage failed:', err); return null }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!slug || !password) return NextResponse.json({ error: 'Missing slug or password' }, { status: 400 })
    ...
    if (!expectedPassword) return NextResponse.json({ error: 'Not found' }, { status: 404 })
    ...
    response.cookies.set(`golfn-auth-${slug}`, token, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', maxAge: 60 * 60 * 24 * 30, path: `/${slug}` })
    return response
