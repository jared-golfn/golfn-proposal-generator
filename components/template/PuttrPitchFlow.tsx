      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
            ...
            <div className="bg-[#1a1f2e] border border-[#2a3347] rounded-2xl overflow-hidden">
              <div className="px-6 md:px-8 py-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div>
                    <div className="flex items-center gap-2 mb-3"><Gift className="w-4 h-4 text-[#00ff9d]" /><h4 className="text-sm font-semibold text-white">The Prize</h4></div>
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-3"><Video className="w-4 h-4 text-[#00ff9d]" /><h4 className="text-sm font-semibold text-white">The Unveiling</h4></div>
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-3"><Zap className="w-4 h-4 text-[#00ff9d]" /><h4 className="text-sm font-semibold text-white">The Second Spin</h4></div>
        ...
        <Fade delay={0.08}>
          <div className="mb-10">
            ...
            <div className="bg-[#1a1f2e] border border-[#2a3347] rounded-2xl overflow-hidden">
              <div className="px-6 md:px-8 py-6">
                <p className="text-base text-[#d1d5db] leading-7 mb-6">Everyone who watched the PUTTR video is now a tagged intent cohort inside GolfN. We hit them with an aggressive, multi-channel follow-up campaign to drive purchases through the Points Exchange marketplace.</p>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="bg-[#0f1217] border border-[#2a3347]/60 rounded-xl p-5"><Zap className="w-5 h-5 text-[#00ff9d] mb-2" /><p className="text-sm font-semibold text-white mb-1">In-App Messages</p><p className="text-xs text-[#9ca3af]">Targeted IAMs to video watchers. Miura campaign hit 43% conversion rate on IAMs.</p></div>
                  <div className="bg-[#0f1217] border border-[#2a3347]/60 rounded-xl p-5"><Target className="w-5 h-5 text-[#00ff9d] mb-2" /><p className="text-sm font-semibold text-white mb-1">Push Notifications</p><p className="text-xs text-[#9ca3af]">Sequenced pushes to the intent cohort with product education and urgency.</p></div>
                  <div className="bg-[#0f1217] border border-[#2a3347]/60 rounded-xl p-5"><ShoppingBag className="w-5 h-5 text-[#00ff9d] mb-2" /><p className="text-sm font-semibold text-white mb-1">Marketplace Placement</p><p className="text-xs text-[#9ca3af]">Featured banner + product listing in Points Exchange. PUTTR available for purchase with points.</p></div>
                  <div className="bg-[#0f1217] border border-[#2a3347]/60 rounded-xl p-5"><Eye className="w-5 h-5 text-[#00ff9d] mb-2" /><p className="text-sm font-semibold text-white mb-1">Ongoing Impressions</p><p className="text-xs text-[#9ca3af]">PUTTR stays in PYL for the full campaign window. 205K+ views per month, compounding.</p></div>
                </div>
              </div>
            </div>
          </div>
        ...
        <Fade delay={0.1}>
          <div className="bg-[#001a14]/60 border border-[#00ff9d]/20 rounded-2xl p-8 md:p-10">
            <p className="text-xl md:text-2xl font-bold text-white leading-relaxed mb-3">A $699 product is not an impulse buy. <span className="text-[#00ff9d]">Repeated exposure is what closes it.</span></p>
            <p className="text-base text-[#9ca3af] leading-7">PYL users come back 7 times a month. Over two months, that is 14 touchpoints per user. They see PUTTR every time. The 15-second unveiling creates awareness. The opt-in video creates intent. The commerce campaign closes the sale. This is not a one-off impression. It is a persistent, multi-touch funnel inside a closed ecosystem of verified golfers.</p>
          </div>
        </Fade>
      </div>
    </section>
...
function MediaValue() {
  return (
    <section className="py-16 md:py-20">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <Fade>
          <div className="flex items-center gap-3 mb-4"><DollarSign className="w-5 h-5 text-[#00ff9d]" /><p className="text-base md:text-lg font-mono tracking-[0.2em] uppercase text-[#00ff9d]">Media Value</p></div>
          <h2 className="text-3xl md:text-4xl lg:text-[2.75rem] font-bold leading-[1.05] tracking-tight mb-4">Your media buy <span className="text-gradient">is the product itself</span></h2>
          <p className="text-lg text-[#9ca3af] max-w-3xl leading-8 mb-8">4 PUTTRs at wholesale is roughly $1,960. Over a two-month campaign, GolfN delivers 1,000,000+ total impressions across PYL, marketplace banners, IAMs, push, and email. That math does not exist anywhere else in golf.</p>
        </Fade>
        <Fade delay={0.04}>
          <div className="bg-[#1a1f2e] border border-[#2a3347] rounded-2xl overflow-hidden mb-8">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-[#2a3347]"><th className="py-4 px-6 text-left text-xs font-mono text-[#6b7280] uppercase tracking-wider">Channel</th><th className="py-4 px-5 text-left text-xs font-mono text-[#6b7280] uppercase tracking-wider">CPM</th><th className="py-4 px-5 text-left text-xs font-mono text-[#6b7280] uppercase tracking-wider">Audience Quality</th></tr></thead>
                <tbody>
                  {cpmRows.map((row, i) => (
                    <tr key={row.channel} className={`${i < cpmRows.length - 1 ? 'border-b border-[#2a3347]/40' : ''} ${row.good === true ? 'bg-[#001a14]/30' : ''}`}>
                      ...
                      <td className="py-4 px-5"><div className="flex items-center gap-2"><CpmIcon good={row.good} /><span className={`text-sm ${row.good === true ? 'text-white font-medium' : 'text-[#9ca3af]'}`}>{row.audience}</span></div></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <Fade delay={0.06}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-[#0f1217] border border-[#2a3347]/60 rounded-xl p-6 text-center"><p className="text-3xl font-mono font-bold text-[#00ff9d] mb-1">$4.78</p><p className="text-sm text-white font-semibold">CPM on PYL Alone</p><p className="text-xs text-[#6b7280] mt-1">410K+ views over 2 months / $1,960 wholesale</p></div>
            <div className="bg-[#001a14]/60 border border-[#00ff9d]/20 rounded-xl p-6 text-center"><p className="text-3xl font-mono font-bold text-[#00ff9d] mb-1">$1.96</p><p className="text-sm text-white font-semibold">Full-Campaign CPM</p><p className="text-xs text-[#6b7280] mt-1">1M+ impressions across all channels over 2 months</p></div>
            <div className="bg-[#0f1217] border border-[#2a3347]/60 rounded-xl p-6 text-center"><p className="text-3xl font-mono font-bold text-[#00ff9d] mb-1">10 units?</p><p className="text-sm text-white font-semibold">Longer campaign, same CPM</p><p className="text-xs text-[#6b7280] mt-1">More prizes extends the window. More winners, more social proof. The audience keeps growing 30% MoM.</p></div>
          </div>
        </Fade>
      </div>
    </section>
    ...
    <section id="the-deal" className="py-20 md:py-28">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <Fade>
          ...
          <p className="text-lg md:text-xl text-[#9ca3af] max-w-3xl mb-10 leading-8">No startup fees. No retainers. No media buy minimums. PUTTR provides product. GolfN provides 30,000 verified golfers and the full execution stack to convert them.</p>
        </Fade>
        <Fade delay={0.04}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
            <div className="bg-[#1a1f2e] border border-[#2a3347] rounded-2xl p-8">
              ...
              <div className="space-y-6">{puttrBrings.map((item) => (<div key={item.title} className="flex gap-4"><div className="flex items-center justify-center w-10 h-10 rounded-xl bg-[#00ff9d]/10 shrink-0"><item.icon className="w-5 h-5 text-[#00ff9d]" /></div><div><p className="text-base font-semibold text-white mb-1">{item.title}</p><p className="text-sm text-[#9ca3af] leading-6">{item.detail}</p></div></div>))}</div>
        ...
        <Fade delay={0.05}>
          <div className="bg-[#1a1f2e] border border-[#2a3347] rounded-2xl overflow-hidden mb-10">
            <div className="px-6 md:px-8 py-5 border-b border-[#2a3347]"><p className="text-xs font-mono text-[#6b7280] uppercase tracking-[0.2em]">How GolfN Compares</p></div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-[#2a3347]"><th className="py-4 px-6 text-left text-xs font-mono text-[#6b7280] uppercase tracking-wider w-[180px]"></th><th className="py-4 px-5 text-left text-xs font-mono text-[#6b7280] uppercase tracking-wider">Traditional Sponsorship</th><th className="py-4 px-5 text-left text-xs font-mono text-[#6b7280] uppercase tracking-wider">Paid Social Ads</th><th className="py-4 px-5 text-left text-xs font-mono text-[#00ff9d] uppercase tracking-wider bg-[#001a14]/30">GolfN Activation</th></tr></thead>
                <tbody>
        ...
        <Fade delay={0.06}>
          <div className="bg-[#001a14]/60 border border-[#00ff9d]/20 rounded-2xl p-8 md:p-10">
            <p className="text-xl md:text-2xl font-bold text-white leading-relaxed mb-3">PUTTR invests <span className="text-[#00ff9d]">~$2,000 in product</span>. GolfN delivers <span className="text-[#00ff9d]">1,000,000+ impressions to verified golfers at under $2 CPM</span>.</p>
            <p className="text-base text-[#9ca3af] leading-7">Imagine what 10 units would do. The campaign runs longer. More winners means more social proof. And the audience keeps growing 30% every month.</p>
          </div>
        </Fade>
      </div>
    </section>
...
const faqs = [
  { q: 'How does the video mechanic work?', a: 'When PYL launches with PUTTR as a prize, every user sees a 15-second unveiling video on their first visit. They get one free spin automatically. To unlock a second free spin, they watch a longer PUTTR product video. This is a one-time offer per user, which means the longer video functions as an intent signal: everyone who watches it is self-selecting as interested in PUTTR.' },
  { q: 'Do we get category exclusivity?', a: 'Yes. GolfN will not run a competing smart putting system or putting mat in PYL or the marketplace simultaneously. Your campaign gets undivided attention from the audience.' },
  { q: 'What happens after the PYL campaign?', a: 'The intent cohort is permanent. Everyone who watched your video, played PYL for a PUTTR, or viewed your marketplace listing is tagged and reactivatable at any time. You can run seasonal campaigns, new product launches, or subscription promotions against this cohort without rebuilding from scratch.' },
  { q: 'What if we want to increase the prize count later?', a: 'More prizes means a longer campaign window, more winners generating social proof, and more reasons to re-engage the cohort with push and email. The impression volume is driven by audience size, not prize count, so your effective CPM actually improves at scale.' },
]
    ...
    <section id="next-steps" className="py-20 md:py-28">
      <div className="max-w-7xl mx-auto px-6 md:px-12 text-center">
        <Fade>
          ...
          <h2 className="text-3xl md:text-5xl font-bold leading-[1.05] tracking-tight mb-6">{"Let's get PUTTR"}<br /><span className="text-[#00ff9d]">in front of 30,000 golfers</span></h2>
          <p className="text-lg text-[#9ca3af] max-w-2xl mx-auto mb-8 leading-8">We handle everything. You provide the product. The first campaign can be live within weeks.</p>
