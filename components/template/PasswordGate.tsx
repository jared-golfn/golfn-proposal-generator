...
interface PasswordGateProps {
  slug: string
  partnerName: string
  /** Badge pill text — defaults to "Protected Proposal" */
  badgeLabel?: string
  /** Main heading — defaults to "Partnership Proposal" */
  title?: string
  /** Submit button label — defaults to "View Proposal" */
  buttonLabel?: string
  /** Footer disclaimer — defaults to standard confidential line */
  footerText?: string
}
...
export function PasswordGate({
  slug,
  partnerName,
  badgeLabel = 'Protected Proposal',
  title = 'Partnership Proposal',
  buttonLabel = 'View Proposal',
  footerText = 'Confidential document. Reach out to Jared for access.',
}: PasswordGateProps) {
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="text-center mb-10">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#1a1f2e] border border-[#2a3347] mb-6">
            ...
            <span className="text-sm font-mono text-[#9ca3af] uppercase tracking-wider">{badgeLabel}</span>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={loading || !password.trim() || !email.trim()} className="w-full flex items-center justify-center gap-2 px-6 py-4 rounded-xl font-semibold text-lg transition-all hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed glow-green" style={{ background: 'linear-gradient(135deg, #00ff9d, #17A455)', color: '#0f1217' }}>
            {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><span>{buttonLabel}</span><ArrowRight className="w-5 h-5" /></>}
          </button>
        </form>
        <p className="text-center text-sm text-[#4b5563] mt-8">{footerText}</p>
      </div>
    </div>
  )
}
