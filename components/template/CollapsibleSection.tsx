...
export function CollapsibleSection({ number, label, title, subtitle, children, defaultOpen = false, id }: CollapsibleSectionProps) {
    ...
    <div id={id} className="border-b border-[#2a3347]/50">
      ...
      >
        <div className="flex items-center gap-4 min-w-0">
          {number && (
            <span className="text-sm font-mono font-bold text-[#00ff9d] bg-[#00ff9d]/10 border border-[#00ff9d]/30 rounded-full w-8 h-8 flex items-center justify-center shrink-0">{number}</span>
          )}
          <div className="min-w-0">
            {label && <p className="text-xs md:text-sm font-mono tracking-[0.2em] uppercase text-[#00ff9d] mb-1">{label}</p>}
