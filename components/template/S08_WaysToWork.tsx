              ...
              <table className="w-full text-left">
                ...
                <tbody>
                  {fixedCosts.map((row) => (
                    <tr
                      key={row.tier}
                      className={`border-t border-[#2a3347]/60 transition-all duration-200 hover:bg-[#1a1f2e]/80 hover:scale-[1.005] ${
                        row.highlight ? 'bg-[#00ff9d]/[0.04]' : 'bg-transparent'
                      }`}
                    >
                      <td className="px-6 py-5">
                        <span className="text-lg font-semibold text-white">{row.tier}</span>
                        {row.highlight && (
                          <span className="ml-2.5 text-[10px] font-mono tracking-wider px-2.5 py-0.5 rounded-full font-bold bg-[#00ff9d] text-[#0f1217] align-middle">
                            POPULAR
                          </span>
                        )}
                      </td>
                ...
                <tbody>
                  {perUserTiers.map((row, i) => (
                    <tr
                      key={row.range}
                      className={`border-t border-[#2a3347]/60 transition-all duration-200 hover:bg-[#1a1f2e]/80 hover:scale-[1.005] ${
                        i === 2 ? 'bg-[#00ff9d]/[0.06]' : 'bg-transparent'
                      }`}
                    >
                      <td className="px-6 py-5 text-lg text-white font-medium">{row.range}</td>
                      <td className="px-6 py-5">
                        <span className="text-2xl font-mono font-bold text-[#00ff9d]">{row.rate}</span>
                      </td>
                      <td className="px-6 py-5 text-base text-[#9ca3af]">
        ...
        <Fade delay={0.18}>
          <div className="flex flex-wrap gap-3 mb-14">
            ...
            {prepayDiscounts.map((d) => (
              <span
                key={d.term}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#00ff9d]/10 border border-[#00ff9d]/30 text-[#00ff9d] text-sm font-semibold transition-all hover:bg-[#00ff9d]/20 hover:scale-105"
              >
