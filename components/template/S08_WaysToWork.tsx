...
export function S08_WaysToWork({ partner }: { partner: PartnerData }) {
  ...
  return (
    <section id="ways-to-work" className="py-20 md:py-32">
            ...
            {pathOrder.map((id) => {
              ...
              return (
                <button
                  ...
                  className={`relative text-left px-6 py-7 rounded-2xl border-2 transition-all duration-300 ${
                    isActive ? 'bg-[#1a1f2e] border-[#00ff9d]/40 shadow-xl scale-[1.02]' : 'bg-transparent border-[#2a3347] hover:bg-[#1a1f2e]/50 hover:border-[#00ff9d]/30'
                  }`}
                >
            ...
            <div className="p-6 md:p-10">
              {scenarioTitle && (
                <div className="mb-8">
                  ...
                  <h3 className="text-2xl font-semibold text-white mb-2">{scenarioTitle}</h3>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {path.scope.map((s) => (
                    <span key={s} className="text-base px-3 py-1.5 rounded-lg bg-[#0f1217] border border-[#2a3347] text-[#d1d5db]">{s}</span>
                  ))}
                </div>
              </div>
            ...
            <div className="border-t border-[#2a3347]">
              <Expand accentColor="#00ff9d" trigger={<span className="text-lg font-semibold text-white">Economics &amp; Pricing Components</span>}>
                <div className="pt-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div><h4 className="text-base font-bold text-white mb-2">Impression Banks</h4><p className="text-base text-[#9ca3af]">Funds delivery volume. Billed as served, no expiration. Separate from management.</p></div>
                  <div><h4 className="text-base font-bold text-white mb-2">Performance Layer</h4><p className="text-base text-[#9ca3af]">Realized only when qualifying downstream actions occur. Aligned incentives.</p></div>
                </div>
                  ...
                  <div className="pt-2 space-y-3">
                    {partner.commerceNotes.map((note, i) => (
                      <div key={i} className="flex items-start gap-3">
                        ...
                        <p className="text-base text-[#d1d5db] leading-7">{note}</p>
                      </div>
                    ))}
                  </div>
                </Expand>
