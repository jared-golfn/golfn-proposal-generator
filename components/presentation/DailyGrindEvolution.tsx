...
export function DailyGrindEvolution({ partner }: { partner: PartnerConfig }) {
  return (
    <section className="w-content px-5 md:px-12 py-20 md:py-32">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start mb-16">
        <div>
          <Fade>
            ...
            <h2 className="font-display text-3xl md:text-6xl mt-3 mb-6 leading-[0.95]">Daily Grind<br /><span className="text-gradient">Evolution</span></h2>
            <p className="text-base md:text-[17px] text-[#B0B0B4] leading-[1.75] mb-6">GolfN's real-world, location-based check-in system. Golfers check in at courses, pro shops, events, Topgolf, PGA Tour events, and partner retail locations.</p>
        ...
        <Fade delay={0.3}>
          <div className="flex justify-center">
            <div className="relative">
              ...
              <img src={images.dailyGrind} alt="Daily Grind" className="relative w-48 md:w-72 rounded-[28px] max-h-[540px] object-cover object-top" style={{ filter: 'drop-shadow(0 32px 64px rgba(0,0,0,0.6))', transform: 'rotate(2deg)' }} />
            </div>
          </div>
        </Fade>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 mb-16">
        {dailyGrindPhases.map((p, i) => (
          <Fade key={p.phase} delay={0.1 + i * 0.08}>
            <div className="bg-[#161618] border border-[#2A2A2C] rounded-xl md:rounded-2xl p-6 md:p-8 relative">
              ...
              <h3 className="text-lg md:text-xl font-semibold mt-4 mb-3">{p.name}</h3>
              <p className="text-sm md:text-[15px] text-[#8C8C8C] leading-relaxed mb-5">{p.description}</p>
      ...
      <Fade delay={0.4}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-10 items-center bg-[#161618] border border-[#2A2A2C] rounded-xl md:rounded-2xl p-6 md:p-10">
          <div>
            <h4 className="text-lg md:text-xl font-semibold mb-3">Learn & Earn — The Digital Complement</h4>
            <p className="text-base md:text-[17px] text-[#B0B0B4] leading-[1.75]">While Daily Grind is real-world and location-based, Learn & Earn is the digital education and comprehension system. It lives within the "More Ways to Earn" section of the app. Users earn points by consuming content and proving comprehension — wrong answers mean no or reduced points. Couch, not course.</p>
          </div>
          <div className="flex justify-center">
            <div className="relative">
              <div className="absolute -inset-6 blur-[60px] opacity-[0.08] rounded-full" style={{ background: partner.secondaryColor }} />
              <img src={images.pointExchange} alt="Point Exchange" className="relative w-44 md:w-64 rounded-[24px] max-h-[440px] object-cover object-top" style={{ filter: 'drop-shadow(0 24px 48px rgba(0,0,0,0.6))' }} />
            </div>
          </div>
        </div>
      </Fade>
