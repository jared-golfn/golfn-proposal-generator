...
function ImageGroupRenderer({ group }: { group: ImageGroup }) {
  if (group.type === 'email') {
    return (
      <div className={`grid gap-4 md:gap-5 ${group.cols === 1 ? 'max-w-md' : ''}`} style={group.cols > 1 ? { gridTemplateColumns: `repeat(${group.cols}, minmax(0, 1fr))` } : {}}>
        {group.images.map((img, idx) => (
          <div key={idx} className="relative rounded-xl overflow-hidden bg-[#0F0F10] border border-[#2A2A2C]">
            <div className="max-h-[600px] overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
              <img src={img} alt="" className="w-full h-auto" loading="lazy" />
            </div>
            <div className="absolute bottom-0 left-0 right-0 h-20 pointer-events-none" style={{ background: 'linear-gradient(to top, #0F0F10, transparent)' }} />
            <div className="absolute bottom-4 left-0 right-0 text-center pointer-events-none">
              <span className="text-xs font-mono text-[#52525B] tracking-wider">SCROLL TO VIEW</span>
            </div>
          </div>
        ))}
      </div>
    ...
    return (
      <div className="max-w-sm">
        <div className="relative rounded-xl overflow-hidden bg-[#0F0F10] border border-[#2A2A2C]">
          <img src={group.images[0]} alt="" className="w-full h-auto max-h-[560px] object-cover object-top" loading="lazy" />
        </div>
      </div>
    )
  }
  return (
    <div className="grid gap-3 md:gap-4" style={{ gridTemplateColumns: `repeat(${Math.min(group.cols, 4)}, minmax(0, 1fr))` }}>
      {group.images.map((img, idx) => (
        <div key={idx} className="relative rounded-xl overflow-hidden bg-[#0F0F10] border border-[#2A2A2C]">
          <img src={img} alt="" className="w-full h-auto" loading="lazy" />
        </div>
  ...
  return (
    <section className="py-20 md:py-32">
      <div className="max-w-[960px] mx-auto px-5 md:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start mb-16 md:mb-20">
          <div>
            <Fade>
              <span className="font-mono text-sm text-[#71717A] tracking-[0.2em] uppercase">Eight Stages</span>
              <h2 className="font-display text-3xl md:text-6xl mt-3 mb-6 leading-[0.95]">Golfer<br /><span className="text-gradient">Progression</span></h2>
              <p className="text-base md:text-[17px] text-[#B0B0B4] leading-[1.75]">
                {selectedPath
          ...
          <Fade delay={0.3}>
            <div className="flex justify-center">
              <div className="relative">
                ...
                <img src={images.offCourse} alt="GolfN app" className="relative w-48 md:w-72 rounded-[28px]" style={{ filter: 'drop-shadow(0 32px 64px rgba(0,0,0,0.6))', transform: 'rotate(2deg)' }} />
              </div>
            </div>
          </Fade>
        </div>
          ...
          <Fade delay={0.15}>
            <div className="flex items-center gap-4 mb-8">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm md:text-base text-[#A1A1AA]">Included in {activePath?.name}</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-sm bg-[#2A2A2C]" />
                <span className="text-sm md:text-base text-[#52525B]">Advanced stage</span>
              </div>
            </div>
          </Fade>
        )}
                  ...
                  <div className="px-5 py-6 md:px-8 md:py-9">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4 md:gap-6">
                        <span className="font-mono text-2xl md:text-4xl font-bold" style={{ color: isDimmed ? '#3A3A3F' : partner.primaryColor, opacity: isDimmed ? 0.5 : opacity }}>{String(stage.number).padStart(2, '0')}</span>
                        <div>
                          <div className="flex items-center gap-2 md:gap-3">
                            <h3 className={`text-lg md:text-2xl font-semibold ${isDimmed ? 'text-[#52525B]' : ''}`}>{stage.name}</h3>
                            {selectedPath && isIncluded && (
                              <span className="text-[9px] md:text-[10px] font-mono tracking-wider px-2 py-0.5 rounded" style={{ background: `${partner.primaryColor}20`, color: partner.primaryColor }}>INCLUDED</span>
                            )}
                            {selectedPath && !isIncluded && (
                              <span className="text-[9px] md:text-[10px] font-mono tracking-wider px-2 py-0.5 rounded bg-[#1A1A1D] text-[#52525B]">ADVANCED</span>
                            )}
                          </div>
                          {!isOpen && <p className={`text-sm md:text-base mt-1 hidden md:block ${isDimmed ? 'text-[#3A3A3F]' : 'text-[#71717A]'}`}>{stage.short}</p>}
                        </div>
                      </div>
                      ...
                      {isOpen && (
                        ...
                        >
                          <div className="mt-6 pt-6 border-t border-[#2A2A2C]">
                            <p className={`text-base md:text-[17px] leading-[1.75] mb-5 ${isDimmed ? 'text-[#52525B]' : 'text-[#B0B0B4]'}`}>{stage.detail}</p>

                            <div className="flex items-center gap-3 mb-5">
                              <span className="text-xs md:text-sm text-[#52525B]">Typically included in:</span>
                              <div className="flex gap-1.5">
                                {pathsForStage.map(p => (
                                  <span
                                    key={p}
                                    className="text-[10px] md:text-xs font-mono tracking-wider px-2 py-0.5 rounded"
                            ...
                            <div className="flex flex-wrap gap-2">
                              {stage.channels.map((ch) => (
                                <span key={ch} className="text-xs md:text-sm font-mono px-3 py-1.5 md:px-4 md:py-2 rounded-full" style={{ background: isDimmed ? '#1A1A1D' : `${partner.primaryColor}12`, color: isDimmed ? '#52525B' : partner.primaryColor }}>{ch}</span>
                              ))}
                            </div>

                            {groups && !isDimmed && (
                              <div className="mt-8 pt-6 border-t border-[#2A2A2C] space-y-10" onClick={(e) => e.stopPropagation()}>
                                {groups.map((group, gi) => (
                                  <div key={gi}>
                                    <p className="text-xs md:text-sm font-mono text-[#71717A] tracking-wider uppercase mb-4 md:mb-5">{group.label}</p>
                            ...
                            {isDimmed && (
                              <div className="mt-6 pt-4 border-t border-[#2A2A2C]">
                                <p className="text-sm md:text-base text-[#52525B] italic">This stage is typically available in {pathsForStage.join(' and ')} paths. Consider upgrading to access this stage.</p>
                              </div>
                            )}
                          </div>
                        </motion.div>
      ...
      <Fade delay={0.4}>
        <div className="flex flex-col items-center mt-8">
          ...
          <div className="mt-2 px-6 py-3 rounded-full border border-[#2A2A2C] bg-[#131315]">
            <span className="text-xs md:text-sm font-mono text-[#71717A] tracking-wide">Fuller-funnel attribution · Measurable progression · Across the golfer journey</span>
          </div>
        </div>
      </Fade>
    </section>
